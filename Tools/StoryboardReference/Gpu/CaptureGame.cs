...
internal static class CaptureEntry
{
    public static int Run(string[] args)
    {
        ...
        var assets = Path.GetFullPath(
            options.GetValueOrDefault("--assets") ?? Path.GetDirectoryName(fixture)!
        );
        ...
        var time = double.Parse(
            options.GetValueOrDefault("--time", "500"),
            CultureInfo.InvariantCulture
        );
        using var host = Host.GetSuitableDesktopHost(
            "ProjectEtherStoryboardGpuOracle",
            new HostOptions
            {
                PortableInstallation = true,
                FriendlyGameName = "Storyboard GPU oracle",
            }
        );
        using var game = new CaptureGame(
            ...
            storage,
            time,
            options.GetValueOrDefault("--hidden", "true") == "true"
        );
        host.Run(game);
...
internal sealed class CaptureGame : Game
{
    ...
    protected override Storage CreateStorage(GameHost host, Storage defaultStorage) =>
        new NativeStorage(storageRoot, host);
    ...
    protected override void LoadComplete()
    {
        ...
        textureStore = new TextureStore(
            captureHost.Renderer,
            captureHost.CreateTextureLoaderStore(
                new StorageBackedResourceStore(new NativeStorage(assetRoot, captureHost))
            ),
            useAtlas: false,
            scaleAdjust: 1
        );
        ...
        foreach (var layer in storyboard.Layers)
        {
            ...
            {
                if (sampleTime < source.StartTime || sampleTime >= source.EndTimeForDisplay)
                    continue;
                if (source.TriggerGroups.Count != 0)
                    throw new NotSupportedException(
                        "Active trigger sprite requires gameplay events."
                    );
                var texture =
                    textureStore.Get(source.Path, WrapMode.ClampToEdge, WrapMode.ClampToEdge)
        ...
        Console.WriteLine(
            $"Ready: {spriteCount} sprites, {captureHost.RendererInfo}, window {Window.ClientSize}"
        );
        _ = Task.Run(async () =>
        {
            ...
            File.WriteAllText(
                ...
                JsonSerializer.Serialize(
                    new
                    {
                        ...
                        width = image.Width,
                        height = image.Height,
                        renderer = captureHost.RendererInfo,
                        gameVersion = typeof(StoryboardSprite)
                            .Assembly.GetName()
                            .Version!.ToString(),
                        frameworkVersion = typeof(Game).Assembly.GetName().Version!.ToString(),
                        hidden,
                        frames,
                        textureStoreUseAtlas = false,
                        textureScaleAdjust = 1,
                        textureFiltering = "Linear",
                        manualMipmaps = false,
                        textureWrap = "ClampToEdge",
                        spriteEdgeSmoothness = "0,0",
                        opaqueBlackBackground = true,
                        viewport = "640x480 centred, scale=height/480, wide viewport unclipped",
                    },
...
internal sealed class OracleSprite : Sprite, IFlippable, IVectorScalable
{
    ...
    public OracleSprite(StoryboardSprite source)
    {
        ...
        Position = source.InitialPosition;
        Name = source.Path;
        LifetimeStart = source.StartTime;
        LifetimeEnd = source.EndTimeForDisplay;
        typeof(Drawable)
            .GetProperty("RemoveCompletedTransforms")!
            .GetSetMethod(true)!
