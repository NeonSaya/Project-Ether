...
try
{
    ...
    for (var i = 0; i < args.Length; i += 2)
    {
        if (i + 1 == args.Length || !args[i].StartsWith("--"))
            throw new ArgumentException(
                "Usage: --input FILE [--times 0,250,500] [--output FILE] [--lazer DIRECTORY] [--format-version 14]"
            );
    ...
    var times = options
        .GetValueOrDefault(
            "--times",
            "-1,0,100,250,500,750,999,1000,1100,1250,1400,1500,1700,2000,2300"
        )
        .Split(',')
        .Select(v => double.Parse(v, CultureInfo.InvariantCulture))
        .ToArray();
    if (times.Any(t => !double.IsFinite(t)))
        throw new ArgumentException("Sample times must be finite.");
    var lazer = Path.GetFullPath(
        options.GetValueOrDefault("--lazer")
    ...
    var game = AssemblyLoadContext.Default.LoadFromAssemblyPath(
        Path.Combine(lazer, "osu.Game.dll")
    );
    var framework = AssemblyLoadContext.Default.LoadFromAssemblyPath(
        Path.Combine(lazer, "osu.Framework.dll")
    );
    ...
    var drawableType = game.GetType(
        "osu.Game.Storyboards.Drawables.DrawableStoryboardSprite",
        true
    )!;
    ...
    var applyAt = drawableType.GetMethod(
        "ApplyTransformsAt",
    ...
    var clockConstructor = framedClockType
        .GetConstructors()
        .Single(c =>
            c.GetParameters().Length > 0
            && c.GetParameters()[0].ParameterType.IsAssignableFrom(manualClockType)
        );

    var format = int.Parse(
        options.GetValueOrDefault("--format-version", "14"),
    ...
    var storyboard = decoderType
        .GetMethod("Decode")!
        .Invoke(decoder, new object[] { reader, Array.CreateInstance(readerType, 0) })!;
    ...
    foreach (var element in Items(Get(layer, "Elements")))
    {
        ...
        if (Items(Get(element, "TriggerGroups")).Any())
            throw new NotSupportedException(
                "Trigger groups require gameplay events and are outside this deterministic oracle."
            );
        ...
        foreach (var time in times)
        {
            ...
            var clockArguments = clockConstructor
                .GetParameters()
                .Select((p, i) => i == 0 ? manualClock : p.DefaultValue)
                .ToArray();
            ...
            samples.Add(
                new
                {
                    ...
                    colour = new
                    {
                        r = Number(Get(colour, "R")),
                        g = Number(Get(colour, "G")),
                        b = Number(Get(colour, "B")),
                        a = Number(Get(colour, "A")),
                    },
    ...
    var result = new
    {
        ...
        limitations = new[]
        {
            "No texture loading, animation-frame selection, rendering, parent hierarchy, masks or gameplay triggers.",
            "Colour is local encoded sRGB; alphaBeforeUpdate preserves raw transform alpha; alpha includes actual sprite overflow correction.",
    ...
    if (options.TryGetValue("--output", out var output))
    {
        ...
        Console.Error.WriteLine(
            $"Wrote {sprites.Count} sprites x {times.Length} samples to {Path.GetFullPath(output)}"
        );
    }
    else
...
static object Vector(object value) =>
    new { x = Number(Get(value, "X")), y = Number(Get(value, "Y")) };
static Dictionary<string, string> Blend(object value) =>
    new[]
    {
        "Source",
        "Destination",
        ...
        name => name == "RGBEquation" ? "rgbEquation" : char.ToLowerInvariant(name[0]) + name[1..],
        name => Get(value, name).ToString()!
    );
