...
public static class StoryboardDisplayConformance
{
    public static string RunOpacity()
    {
        ...
        image.SetPixel(0,0,Color.red);image.Apply();
        ...
        return "PASS whole-frame transparency uses the same RGB/alpha factor as video: "+actual+Environment.NewLine+report;
    }

    public static string Run(string unityFrame, string lazerFrame, string prefix, bool whiteBackground = false)
    {
        ...
        camera.scene=scene;
        camera.enabled=false; camera.orthographic=true; camera.orthographicSize=4.5f;
        camera.transform.position=new Vector3(0,2.5f,0);
        camera.clearFlags=CameraClearFlags.SolidColor;camera.backgroundColor=whiteBackground ? Color.white : Color.black;
        camera.stereoTargetEye=StereoTargetEyeMask.None;camera.allowHDR=false;camera.allowMSAA=false;
        var cameraData=camera.GetUniversalAdditionalCameraData();
        cameraData.renderPostProcessing=false;cameraData.allowXRRendering=false;
        ...
        var input=new RenderTexture(actualSource.width,actualSource.height,0,RenderTextureFormat.ARGB32,RenderTextureReadWrite.Linear);
        input.Create();
        Texture2D actual=null,reference=null;
        try
        {
            foreach(var pixel in actualSource.GetPixels32())
                if(pixel.a!=255) throw new Exception("Use an opaque storyboard frame for the final-display comparison.");
            ...
            if(display==null) throw new Exception("Original screen is hidden by current settings");
            var mesh=display.GetComponent<MeshFilter>().sharedMesh;
            if(Mathf.Abs(mesh.bounds.size.x-12)>0.001f || Mathf.Abs(mesh.bounds.size.y-8)>0.001f || Mathf.Abs(mesh.bounds.size.z-0.36f)>0.001f)
            ...
            GL.sRGBWrite=false; Graphics.Blit(actualSource,input);GL.sRGBWrite=srgb;
            RenderPipeline.SubmitRenderRequest(camera,new UniversalRenderPipeline.SingleCameraRequest{destination=target});
            actual=StoryboardRenderConformance.Read(target);
            GL.sRGBWrite=false;Graphics.Blit(referenceSource,input);GL.sRGBWrite=srgb;
            RenderPipeline.SubmitRenderRequest(camera,new UniversalRenderPipeline.SingleCameraRequest{destination=target});
            ...
            var a=actual.GetPixels32();var b=reference.GetPixels32();int changed=0,max=0;long sum=0;
            int nonBlack=0;
            for(int i=0;i<a.Length;i++)
            {
                if(a[i].r+a[i].g+a[i].b>0)nonBlack++;
                int dr=Math.Abs(a[i].r-b[i].r),dg=Math.Abs(a[i].g-b[i].g),db=Math.Abs(a[i].b-b[i].b);
                int d=Math.Max(dr,Math.Max(dg,db));if(d>0)changed++;max=Math.Max(max,d);sum+=dr+dg+db;
            }
            if(nonBlack<10000)throw new Exception("Display capture did not render the screen");
            string report="Original curved screen 12x8, depth0.36, alpha="+alpha+", source wired through HolographicScreenManager.SetRenderTexture; pixels="+a.Length+", nonBlack="+nonBlack+", different="+changed+", maxChannelDelta="+max+", meanChannelDelta="+(sum/(double)(a.Length*3));
            File.WriteAllText(prefix+"-screen-comparison.txt",report);
            return report;
        }
        finally
        {
            var fields=typeof(HolographicScreenManager).GetFields(BindingFlags.Instance|BindingFlags.NonPublic);
            foreach(var field in fields)
                if(field.FieldType==typeof(Material)||field.FieldType==typeof(Texture2D)||field.FieldType==typeof(Mesh))
                {
            ...
            UnityEngine.Object.DestroyImmediate(actualSource);UnityEngine.Object.DestroyImmediate(referenceSource);
            input.Release();UnityEngine.Object.DestroyImmediate(input);target.Release();UnityEngine.Object.DestroyImmediate(target);
            UnityEngine.Object.DestroyImmediate(cameraObject);UnityEngine.Object.DestroyImmediate(go);
