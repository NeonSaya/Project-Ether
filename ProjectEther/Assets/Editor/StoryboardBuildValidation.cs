...
public static class StoryboardBuildValidation
{
    ...
    {
        "Assets/Shader/SBInstanced.shader",
        "Assets/Shader/SBOverlay.shader",
        "Assets/Shader/SBVideoOverlay.shader",
        "Assets/Shader/HolographicScreen.shader",
    };
    ...
    public static string BuildAndroidShaders()
    {
        ...
        Application.LogCallback listener = (message, stack, type) =>
        {
            if (
                type == LogType.Error
                && (message.Contains("Shader error") || message.Contains("Build failed"))
            )
                errors.Add(message);
        };
        ...
        try
        {
            ...
            {
                assetBundleName = "storyboard-shaders",
                assetNames = Shaders,
            };
            ...
            foreach (var path in Shaders)
            {
                ...
                foreach (var message in ShaderUtil.GetShaderMessages(shader))
                    if (message.severity.ToString() == "Error")
                        errors.Add(path + ": " + message.message + " (" + message.platform + ")");
            }
            string report =
                (result != null && errors.Count == 0 ? "PASS" : "FAIL")
                + ": Android Vulkan/OpenGLES3 storyboard shader compilation; errors="
                + errors.Count
                + Environment.NewLine
