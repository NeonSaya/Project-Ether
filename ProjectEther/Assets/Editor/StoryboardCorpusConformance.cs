...
public static class StoryboardCorpusConformance
{
    public static string Run(string songsDirectory)
    {
        if (StoryboardRenderer.Instance != null)
            throw new InvalidOperationException("Run with an idle Edit Mode editor");
        ...
        Application.LogCallback log = (message, stack, type) =>
        {
            ...
            if (
                ...
                && (message.Contains("[SBParser]") || message.Contains("[SBRenderer]"))
            )
        ...
        try
        {
            ...
            )
            {
                ...
                var timeline = (SBFlatTimelineData)
                    typeof(StoryboardRenderer).GetField("_flatTimeline", flags).GetValue(renderer);
                double first = double.MaxValue,
                    last = 0;
                for (int i = 0; i < timeline.SpriteCount; i++)
                {
                    var sprite = timeline.Sprites[i];
                    if (sprite.EndTime <= sprite.StartTime)
                        continue;
                    first = Math.Min(first, sprite.StartTime);
                    last = Math.Max(last, sprite.EndTime);
                }
                ...
                var textures = (System.Collections.ICollection)
                    typeof(StoryboardRenderer)
                        .GetField("storyboardTextures", flags)
                        .GetValue(renderer);
                output.Add(
                    Path.GetFileName(path)
                        + ": sprites="
                        + timeline.SpriteCount
                        + ", textures="
                        + textures.Count
                        + ", frames=5, warnings="
                        + warnings.Count
