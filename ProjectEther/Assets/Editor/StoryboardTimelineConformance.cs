...
public static class StoryboardTimelineConformance
{
    ...
    [Serializable] public class Sprite { public int index; public string layer, path; public double startTime, endTimeForDisplay; public Sample[] samples; }
    [Serializable] public class XY { public float x, y; }
    [Serializable] public class RGB { public float r,g,b,a; }
    [Serializable] public class Blend { public string destination; }
    [Serializable] public class Sample
    {
        public double time; public XY position, scale, vectorScale;
        public float rotationDegrees, alphaBeforeUpdate, alpha;
        public RGB colour; public bool flipH, flipV; public Blend blending;
    }
    ...
    public static string Compare(string oraclePath, string inputPath = null)
    {
        ...
        string source = inputPath ?? (Path.IsPathRooted(oracle.input) ? oracle.input : Path.Combine(Path.GetDirectoryName(oraclePath), oracle.input));
        var storyboard = StoryboardParser.ParseFile(source);
        var textures = new Dictionary<string,int>();
        var elements = new List<OsuVR.Storyboard.Data.SBElement>(storyboard.GetAllElementsInRenderOrder());
        foreach (var e in elements)
        {
            string key = e.ImagePath.Replace((char)92,'/').ToLowerInvariant();
            if (!textures.ContainsKey(key)) textures[key] = textures.Count;
        }
        var sizes = new Vector2Int[textures.Count];
        for (int i=0;i<sizes.Length;i++) sizes[i]=new Vector2Int(1,1);
        ...
        var failures = new List<string>(); int samples=0, comparisons=0;
        try
        {
            foreach(var expected in oracle.sprites)
            {
                if (expected.layer == "Fail") continue;
                int spriteIndex=elements.FindIndex(e => e.ImagePath == expected.path);
                if(spriteIndex<0) { failures.Add("Missing sprite "+expected.path); continue; }
                foreach(var sample in expected.samples)
                {
                    if(sample.time<expected.startTime || sample.time>=expected.endTimeForDisplay) continue;
                    var job = new SBEvaluateTimelineJob { Sprites=timeline.Sprites, Commands=timeline.Commands,
                        Loops=timeline.Loops, FrameMap=timeline.FrameMap, TriggerCommands=timeline.TriggerCommands, Output=output,
                        CurrentTime=sample.time, SpriteCount=timeline.SpriteCount };
                    job.Schedule(timeline.SpriteCount,64).Complete();
                    var actual=output[spriteIndex]; samples++;
                    ...
                    Compare(actual.ScaleX,sample.scale.x*sample.vectorScale.x,"scaleX",0.0002f);
                    Compare(actual.ScaleY,sample.scale.y*sample.vectorScale.y,"scaleY",0.0002f);
                    Compare(actual.Rotation*Mathf.Rad2Deg,sample.rotationDegrees,"rotation",0.002f);
                    ...
                    Compare(actual.Additive,sample.blending.destination=="One"?1:0,"additive",0);
            ...
            string report="lazer "+oracle.gameVersion+" / framework "+oracle.frameworkVersion+
                ": "+samples+" samples, "+comparisons+" property comparisons, "+failures.Count+" failures";
            string details=report+Environment.NewLine+string.Join(Environment.NewLine,failures);
