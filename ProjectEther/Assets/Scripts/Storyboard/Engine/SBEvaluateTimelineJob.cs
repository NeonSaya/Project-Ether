...
namespace OsuVR.Storyboard.Engine
{
    ...
    public struct SBEvaluateTimelineJob : IJobParallelFor
    {
        ...
        [WriteOnly]
        public NativeArray<SpriteInputData> Output;
        ...
        public void Execute(int i)
        {
            ...
            var state = new SpriteInputData
            {
                X = sprite.InitX,
                Y = sprite.InitY,
                ScaleX = sprite.InitScaleX,
                ScaleY = sprite.InitScaleY,
                VectorScaleX = sprite.InitVectorScaleX,
                VectorScaleY = sprite.InitVectorScaleY,
                Alpha = sprite.InitAlpha,
                Rotation = sprite.InitRotation,
                R = sprite.InitR,
                G = sprite.InitG,
                B = sprite.InitB,
                FlipH = sprite.InitFlipH,
                FlipV = sprite.InitFlipV,
                Additive = sprite.InitAdditive,
            ...
            for (int t = 0; t < selected.Length; t++)
                selected[t] = new Candidate { Index = -1 };
            ...
            for (int l = sprite.LoopOffset; l < sprite.LoopOffset + sprite.LoopCount; l++)
            {
                ...
                for (int c = loop.InnerCmdOffset; c < loop.InnerCmdOffset + loop.InnerCmdCount; c++)
                {
                    ...
                    double iteration =
                        loop.LoopDuration > 0
                            ? math.min(
                                math.floor((CurrentTime - first) / loop.LoopDuration),
                                math.max(1, loop.LoopCount) - 1
                            )
                            : 0;
                    Select(
                        ref selected,
                        c,
                        loop.StartTime + iteration * loop.LoopDuration,
                        false,
                        loop.StartTime
                    );
                }
            ...
            for (
                int c = sprite.TriggerHead;
                c >= 0 && c < TriggerCommands.Length;
                c = TriggerCommands[c].Next
            )
            ...
            for (int t = 0; t < selected.Length; t++)
            {
                ...
                if (candidate.Index >= 0)
                    Apply(
                        candidate.Triggered
                            ? TriggerCommands[candidate.Index].Command
                            : Commands[candidate.Index],
                        CurrentTime - candidate.Offset,
                        ref state,
                        ref uniformScale
                    );
            }
        ...
        )
        {
            ...
            if ((uint)command.Target >= (uint)selected.Length)
                return;
            ...
            if (command.Target >= 7 && command.Target <= 9 && CurrentTime >= end)
                start = end;
            ...
            if (
                ...
                && (
                    ...
                    || (
                        start == previous.Start
                        && (
                            orderStart < previous.OrderStart
                            || (orderStart == previous.OrderStart && orderEnd < previous.OrderEnd)
                        )
                    )
                )
            )
            ...
            if (
                ...
                return;
            selected[command.Target] = new Candidate
            {
                Index = index,
                Start = start,
                End = end,
        ...
        static void Apply(
            SBCommandFlatData command,
            double time,
            ref SpriteInputData state,
        ...
        )
        {
            ...
            switch (target)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    state.ScaleX = valueFloat;
                    break;
                case 4:
                    state.ScaleY = valueFloat;
                    break;
                case 5:
                    state.Rotation = valueFloat;
                    break;
                case 10:
                    uniformScale = valueFloat;
                    break;
                case 11:
                    state.VectorScaleX = valueFloat;
                    break;
                case 12:
                    state.VectorScaleY = valueFloat;
                    break;
            }
        }
        ...
        static float GetEasedProgress(SBCommandFlatData command, double time)
        {
            ...
            return EaseFloat(
                command.Easing,
                (float)((time - command.StartTime) / (command.EndTime - command.StartTime))
            );
        }
        ...
        static float EaseFloat(int easing, float t)
        {
            switch (easing)
            {
                ...
                case 1: // Out (OutQuad)
                case 4:
                    return t * (2f - t); // OutQuad
                case 2: // In (InQuad)
                case 3:
                    return t * t; // InQuad
                case 5:
                    return t < 0.5f ? t * t * 2f : (t - 1f) * (t - 1f) * -2f + 1f; // InOutQuad
                case 6:
                    return t * t * t; // InCubic
                case 7:
                {
                    float n = t - 1f;
                    return n * n * n + 1f;
                ...
                case 8:
                    return t < 0.5f ? t * t * t * 4f : (t - 1f) * (t - 1f) * (t - 1f) * 4f + 1f; // InOutCubic
                case 9:
                    return t * t * t * t; // InQuart
                case 10:
                {
                    float n = t - 1f;
                    return 1f - n * n * n * n;
                ...
                case 11:
                    return t < 0.5f
                        ? t * t * t * t * 8f
                        : (t - 1f) * (t - 1f) * (t - 1f) * (t - 1f) * -8f + 1f; // InOutQuart
                case 12:
                    return t * t * t * t * t; // InQuint
                case 13:
                {
                    float n = t - 1f;
                    return n * n * n * n * n + 1f;
                ...
                case 14:
                    return t < 0.5f
                        ? t * t * t * t * t * 16f
                        : (t - 1f) * (t - 1f) * (t - 1f) * (t - 1f) * (t - 1f) * 16f + 1f; // InOutQuint
                case 15:
                    return 1f - math.cos(t * math.PI * 0.5f); // InSine
                case 16:
                    return math.sin(t * math.PI * 0.5f); // OutSine
                case 17:
                    return 0.5f - 0.5f * math.cos(math.PI * t); // InOutSine
                case 18:
                    return math.pow(2f, 10f * (t - 1f)) + (t - 1f) / 1024f; // InExpo
                case 19:
                    return -math.pow(2f, -10f * t) + 1f + t / 1024f; // OutExpo
                case 20: // InOutExpo
                    return t < 0.5f
                        ? 0.5f * (math.pow(2f, 20f * t - 10f) + (2f * t - 1f) / 1024f)
                        : 1f - 0.5f * (math.pow(2f, -20f * t + 10f) + (-2f * t + 1f) / 1024f);
                case 21:
                    return 1f - math.sqrt(1f - t * t); // InCirc
                case 22:
                {
                    float n = t - 1f;
                    return math.sqrt(1f - n * n);
                } // OutCirc
                case 23: // InOutCirc
                {
                    float n = t * 2f;
                    if (n < 1f)
                        return 0.5f - 0.5f * math.sqrt(1f - n * n);
                    n -= 2f;
                ...
                case 24: // InElastic
                    return -math.pow(2f, -10f + 10f * t)
                        ...
                        + (1f - t) / 2048f;
                case 25: // OutElastic
                    return math.pow(2f, -10f * t) * math.sin((t - 0.075f) * (2f * math.PI) / 0.3f)
                        + 1f
                        - t / 2048f;
                case 26: // OutElasticHalf
                    return math.pow(2f, -10f * t)
                            * math.sin((0.5f * t - 0.075f) * (2f * math.PI) / 0.3f)
                        + 1f
                        - t / 1024f * math.sin((0.5f - 0.075f) * (2f * math.PI) / 0.3f);
                case 27: // OutElasticQuarter
                    return math.pow(2f, -10f * t)
                            * math.sin((0.25f * t - 0.075f) * (2f * math.PI) / 0.3f)
                        + 1f
                ...
                case 28: // lazer 的 InOutElastic 使用 0.45 的周期，并做端点校正。
                {
                    ...
                    if (n < 1f)
                        return -0.5f
                            * (
                                math.pow(2f, -10f + 10f * n)
                                    * math.sin((1f - 0.1125f - n) * frequency)
                                - offset * (1f - n)
                            );
                    n -= 1f;
                    return 0.5f
                            * (
                                math.pow(2f, -10f * n) * math.sin((n - 0.1125f) * frequency)
                                - offset * n
                            )
                        + 1f;
                }
                case 29:
                    return t * t * ((1.70158f + 1f) * t - 1.70158f); // InBack
                case 30:
                {
                    float n = t - 1f;
                    return n * n * ((1.70158f + 1f) * n + 1.70158f) + 1f;
                } // OutBack
                case 31: // InOutBack
                {
                    float n = t * 2f;
                ...
                case 32:
                    return 1f - EaseBounce(1f - t); // InBounce
                case 33:
                    return EaseBounce(t); // OutBounce
                case 34: // InOutBounce
                    return t < 0.5f
