...
namespace OsuVR.Storyboard
{
    ...
    public sealed class StoryboardPlayerCapture : MonoBehaviour
    {
        ...
        static void StartIfRequested()
        {
            ...
            if(flag<0 || flag+3>=args.Length) return;
        ...
        IEnumerator Start()
        {
            ...
            try
            {
                ...
                if(renderer==null)throw new InvalidOperationException("Storyboard renderer did not initialise");
                renderer.enabled=false;
                renderer.LoadStoryboard(StoryboardParser.ParseFile(mapPath),Path.GetDirectoryName(mapPath),true);
                ...
                string report="PASS player capture: "+SystemInfo.graphicsDeviceType+"; "+rt.width+"x"+rt.height+"; time="+sampleTime;
