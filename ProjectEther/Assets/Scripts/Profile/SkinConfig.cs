    ...
    public class SkinConfig : ScriptableObject
    {
        ...
        )
        {
            switch (set)
            {
                case SampleSet.Drum:
                    if (isSliderSlide)
                        return drum_sliderslide;
                    if (isSliderTick)
                        return drum_slidertick;
                    if ((type & HitSoundType.Finish) != 0)
                        return drum_hitfinish;
                    if ((type & HitSoundType.Whistle) != 0)
                        return drum_hitwhistle;
                    if ((type & HitSoundType.Clap) != 0)
                        return drum_hitclap;
                    return drum_hitnormal;

                case SampleSet.Soft:
                    if (isSliderSlide)
                        return soft_sliderslide;
                    if (isSliderTick)
                        return soft_slidertick;
                    if ((type & HitSoundType.Finish) != 0)
                        return soft_hitfinish;
                    if ((type & HitSoundType.Whistle) != 0)
                        return soft_hitwhistle;
                    if ((type & HitSoundType.Clap) != 0)
                        return soft_hitclap;
                ...
                default:
                    if (isSliderSlide)
                        return normal_sliderslide;
                    if (isSliderTick)
                        return normal_slidertick;
                    if ((type & HitSoundType.Finish) != 0)
                        return normal_hitfinish;
                    if ((type & HitSoundType.Whistle) != 0)
                        return normal_hitwhistle;
                    if ((type & HitSoundType.Clap) != 0)
                        return normal_hitclap;
